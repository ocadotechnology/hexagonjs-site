  ...
  if (content2.some(function (e) { return typeof (e) === 'string' && e !== '' })) {
    ...
    content2.forEach(function (e2) {
      ...
      if (!!e1) {
        ...
        if (isTaggable) {
          if (!e2.content.some(({type}) => type === 'added')) {
...
function versionTransform (obj, options) {
  ...
  if (actualVersions.length > 0) {
    ...
    actualVersions.forEach(function (versionSel) {
      const version = versionSel.ps()

      replaceTags(versionSel, options.tags, version)

      merged.content = mergeContent(merged.content, versionSel.content, options, version)
    })
    removeVersions(merged)
    return [fixTags(fixDescription(merged), Object.keys(options.tags), fullVersionList)]
